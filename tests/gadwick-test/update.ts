        ...
        for (const gadwickFeature of features)
        {
            ...
            else
            {
                ...
                [
                    ...
                    `describe('${gadwickFeature.feature_name}', function() {`,
                    `\tit('${gadwickFeature.description}', function() {`,
                    `\t\t// Test steps here. See the gadwick site for examples.`,
                    `\t})`,
                    `})`
                ]
                idMap.push({ id: gadwickFeature["feature-id"], name: gadwickFeature.feature_name });
                fs.writeFile(path.join(testSuiteDirectoryPath, `${gadwickFeature.feature_name.replace(" ", "")}.spec.js`), fileData.join("\n"), (err) => {
                    if (err) throw err;
